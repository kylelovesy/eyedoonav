...
```typescript
const { profile, state, loading, error, fetchProfile, updateProfile, refresh } = useUserProfile(
  userId,
  { autoFetch: true },
);
...
```typescript
const { preferences, state, loading, error, fetchPreferences, updatePreferences, refresh } =
...
```typescript
const { subscription, state, loading, error, fetchSubscription, updateSubscription, refresh } =
